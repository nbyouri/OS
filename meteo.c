...
int gen_atis(void){
    ...
    close(fichierTexte);
    close(fichierLock);
    if(unlink(FICHIERLOCK) == -1){
			printf("Impossible de detruire le fichier lock");
			return EXIT_FAILURE;
    }
